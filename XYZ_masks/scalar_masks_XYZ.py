...
class Scalar_mask_XYZ(Scalar_field_XYZ):
    ...
    def prism(self, r0, refractive_index, length, height, lower_base, upper_base):
        ...
        and the height of the abscissa is parallel with the x axis.

        Parameters:
            r0 (float, float, float): (x0, y0,z0) Location of the prism, for example (0*um, 0*um, 0*um)
            length (float): Length of prism
            refractive_index (float, str): Refraction index , for example: 1.5 + 1.0j
            triangle_height: The height of the triangle measured from the basis
