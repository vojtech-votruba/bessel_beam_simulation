...
from diffractio.config import CONF_DRAWING
from diffractio.utils_common import get_date, load_data_common, save_data_common
from diffractio.utils_drawing import (draw2D, normalize_draw, prepare_drawing,
                            reduce_matrix_size)
from diffractio.utils_math import (get_edges, get_k, nearest, nearest2,
...
class Scalar_field_XY(object):
    ...
    def __init__(self, x=None, y=None, wavelength=None, info=""):
        ...
        if x is not None and y is not None:
            self.X = np.memmap("X_gridsource.dat", dtype=np.float64, mode="w+", shape=(x.size, x.size))
            self.Y = np.memmap("Y_gridsource.dat", dtype=np.float64, mode="w+", shape=(y.size, y.size))
            ...
            for start in range(0, len(y), chunk_size):
                ...
                temp_x = np.tile(x, (end - start, 1))
                temp_y = np.repeat(y[start:end], len(x)).reshape(end - start, len(x))
                
                self.X[start:end, :] = temp_x
                self.Y[start:end, :] = temp_y
                
            self.X.flush()
            self.Y.flush()

            self.u = np.memmap("u_gridsource.dat", dtype=complex, mode="w+", shape = (self.X))
            self.u[:] = 0
            self.u.flush()
            #= zeros(shape(self.X), dtype=complex)
        else:
            self.X = None
            self.Y = None
            self.u = None
